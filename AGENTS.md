...
J-space 검증: PDF 93쪽·인용 155개 bibtex 해석 / HTML raw `[cite:` 0·csl-entry 173·citation span 정상·MathJax.
...
= `/anthropic-serve/`(distill+katex, 범용) + `/2026/workspace/`(bundle.js=논문 전용). 캡슐 산출물=`out/`(gitignore, 원문 저작권=Anthropic).

**org→인터랙티브 HTML**(`paper2org-interactive`, `anthropic_paper_to_org.py --interactive`): GLG 명제 실증 —
**org=SSOT, LaTeX/Typst/Quarto 없이 pandoc 만으로 인터랙티브 문서 생성.** 인터랙티브 figure 를 원본 `<figure
data-fignum>` outerHTML verbatim 으로 `#+begin_export html` 블록에 보존(수식/인용보다 먼저 protect → figcaption
custom element pristine), 원문 head 런타임을 `#+HTML_HEAD_EXTRA`, `<d-article>`/`<d-contents>` 래퍼도 org raw 블록
(=pandoc template 파일 0). pandoc `--katex=/anthropic-serve/katex/`(로컬, 외부요청 0). 산출물 = 캡슐 트리 안
`capsule/<url-path>/<name>.interactive.html`, docroot=capsule 로 열면 bundle.js 가 figure hydrate. jspace 실증:
figure 84, 외부요청 0, parquet 39 로컬 fetch, 콘솔 에러 0. SSOT=interactive.org+manifest. prose 컬럼 폭 폴리시=NEXT.
...
./run.sh paper2org-capsule <URL> [--name NAME] [--no-verify] # → <name>/capsule/ (오프라인 hydrate 캡슐)
./run.sh paper2org-interactive <URL> [--name NAME]      # → capsule/<url-path>/<name>.interactive.html (org→인터랙티브)
