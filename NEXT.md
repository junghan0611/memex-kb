...
### 자연철학강의 — 봉합 적용 완료 ✅ (2026-06-04, 둘째 책)

config `paragraph_merge.enabled:true`, **`categories:["page_boundary"]`만** → **187봉합 / 11skip / 0miss**,
**EPUB epubcheck 0/0/0**(6.4MB). 구조(장10/절30/소절122)·section_markers 30·운문 행·도식 박스 보존.
**org 바이트 동일 재현 검증 완료**(merges.log md5 동일). seam josa-completion 3건(`에+서→에서` ×2, `에+게→에게`) literal 교정.

⭐ **새 노하우 — samepage_break는 책마다 판단**(영속 가치, scanbook 스킬에 환류 필요):
물질생명인간은 page_boundary+samepage 둘 다 안전했지만, **자연철학강의는 samepage_break 제외**가 정답.
dry-run(264봉합) 통독에서 samepage ~77건이 이 책 특유 구조 요소를 오봉합 — **십우도 운문**(시 행 nospace 붙음),
**한문 원문 인용**(`人爲能知…` 본문 흡수), **도식 박스 라벨**(`[지식1][정보1]…` 체인 봉합), **수식 변수 범례**,
**그림 캡션**, **아인슈타인 광고**. page_boundary는 페이지경계 평문연속만이라 운문/한문/라벨 0건.
→ **교훈: 운문·도식·한문 밀도 높은 책은 samepage_break 끄고 page_boundary만. 항상 dry-run merges.log 통독 후 결정.**

### 물리학강의 — 봉합 적용 완료 ✅ (2026-06-04, 셋째 책)

config `paragraph_merge.enabled:true`, **`categories:["page_boundary","samepage_break"]` 둘 다** →
**430봉합 / 66skip / 0miss**, **EPUB epubcheck 0/0/0**(9.9MB). 구조(부7/강27/소절129)·강표지 4·13·18강 보존.
**org 바이트 동일 재현 검증 완료**(merges.log md5 동일).

⭐ **핵심 노하우 — samepage 오염을 통째 제외 대신 skip 규칙으로 정화**(영속, `_seam_for` SSOT):
자연철학강의는 samepage 통째 제외가 답이었지만, 물리학강의 오염은 *규칙적인 3클래스*라 skip 규칙을 가르쳐 둘 다 켰다.
`mineru2org.py` `_seam_for`에 추가(모든 책 공통):
- **caption-head**: head `그림/표/사진 N-N:` 도판 캡션 → skip (5건)
- **pageref-tail**: tail `…NNN쪽` 교차참조 라벨(`더 알아보기 ① … 202쪽`) → skip (19건). `\d쪽$`라 왼쪽/양쪽 면제
- **enum-circled**: tail `①` 시작 + **≤24자** 박스 소제목(`① 로렌츠 변환식`) → skip (21건).
  ⚠️ **길이 게이트 ≤24가 load-bearing**: 물리학강의 박스제목 ≤21자 vs 물질생명인간 ①-칸트인용 ≥27자 → 둘을 가름. 게이트 없으면 칸트 평문 봉합이 회귀.
- **회귀 0 검증**: `_seam_for`는 3책 공통 SSOT라, 규칙 추가 후 자연철학강의·물질생명인간 org 바이트 동일 재확인 필수 → 통과.
→ **교훈: 오염이 자유형(운문/한문)이면 카테고리 드롭, 규칙형(캡션/교차참조/박스제목)이면 skip 규칙 추가해 평문 살림. 새 skip 규칙은 항상 커밋된 책 회귀 0 확인.**

### 다음 한 걸음 — 나머지 책 + 백로그

- [ ] **다음 책**: 물리의정석 / 인공지능시대와 철학의 쓸모 — scanbook New book checklist로 시작.
- [ ] eq/image_interrupt는 여전히 보류(수동). image 뒤 블록이 캡션/변수설명인 경우 구분 필요.
- [x] **scanbook SKILL.md 환류 완료** — §Reliability tiers를 samepage 결정 fork(드롭 vs skip규칙)로 정련, `_seam_for` 규칙 순서에 caption-head/pageref-tail/enum-circled 추가(2026-06-04).
- [ ] (선택) 봉합 워크플로 스킬화 — 리스트→봉합→로그검수→환류 루프.
- [ ] 물리학강의 candidates.log 858건 LLM 경량 패스(저우선, 본문 글자정확도만 남음 — 구조/EPUB 완료).
