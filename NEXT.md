...
**만들 것 = 브리지 (paper2org org → export-ready org)**. paper2org 는 평문 `#+title`/`#+author` N명 + `[cite:@key]`
+ `#+bibliography` 만 낸다(=web/pandoc 친화). acmart PDF 로 가려면 헤더를 변환해야 함:
1. **arxiv PDF**: `#+OPTIONS: title:nil author:nil` + `#+LATEX_CLASS: acmart [sigconf,nonacm]` + `#+BEGIN_EXPORT latex`
   블록 **자동생성**(paper2org authors 리스트 → `\title` + N명 `\author/\affiliation` + `\maketitle`). 인용은 acmart용
   `#+cite_export: natbib`(+bibliographystyle). 수식 `\(..\)`/`\[..\]`·이미지 그대로. 인터랙티브 캡션 마커 → 본문 텍스트.
2. **web HTML**: ox-html + org-cite citeproc(인용까지 렌더) → Distill 유사 웹 아티클. pandoc 왕복=증명, ox-html=프로덕션.
3. (선택) **docx**: pandoc org→docx 또는 기존 md_to_gdocs 경로.

**설계 결정**: 브리지를 paper2org `--export` 옵션 심기 vs 별도 `paper-export <org> --to pdf,html` 명령 → **별도 명령
추천**(paper2org=캡처, export=파생 분리). acmart 초록 필수 아님(J-space는 초록 없이 Intro 시작 → abstract 생략/첫문단).
PDF/HTML 산출물도 `out/` gitignore(Anthropic 원문).

**검증**: `out/anthropic-paper/jspace/jspace.org` → PDF 1편 실제 빌드(latexmk 다중컴파일+인용 해석) + HTML 1편(인용 렌더).
texlive scheme-full 필요(arxiv-acm/flake). 참고: `geworfen/docs`(build.el·Makefile·sample.org acmart 패턴), `run.sh arxiv-build`.
