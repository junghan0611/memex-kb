...
### 다음 한 걸음
- [ ] **실전 1건** — 실제 autholog 노트로 묶음 만들어 복붙/브라우저까지 돌려보고 포맷 검증(특히 네이버 서식·요약형 글자수).
- [ ] 요약형 글자수 초과 시 에이전트가 매체별로 자연스럽게 줄이는 루프(현재는 입력 요약 그대로).
- [ ] 새 노트 금지·기존 확장 매칭 보조(denotecli/semantic-memory)를 스킬 워크플로에 실제 연결.
- [ ] (후속) API-clean 자동발행 — Threads(`threads_content_publish` 보유) → Bluesky(AT Proto). 읽기/쓰기 스킬 흡수.
- [ ] (후속) 네이버/티스토리/개인 페북 브라우저 자동화(`browser-tools`/크롬확장) — 묶음 파일 소비.
- [ ] AGENTS.md/README/BACKENDS 동기화(syndicate 명령·스킬). 봇 군단 주입.

> ⚠️ 검토 메모: 면들이 **API-clean(Threads·Bluesky·FB Page·IG) vs 브라우저-자동화-강제(네이버·티스토리·개인FB)** 두 종족. 후속 자동화 시 이 분기 유지. 포맷 규격 변경은 `PLATFORMS` 한 곳만.
