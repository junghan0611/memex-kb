...
2. [x] (2026-07-07) **web HTML 완성** — `paper2org-html` = **pandoc --citeproc**(org-cite 파싱 → (Author Year)+참고문헌).
   emacs·texlive 불필요. 검증: raw `[cite:` 0, **csl-entry 173(참고문헌 채워짐)**, citation span 157, 수식·이미지.
   ⚠️ ox-html(oc-basic)은 실무 bib(중복키/필수필드)에서 `bibtex-validate` 실패 → 인용 전부 `??`·빈 참고문헌 → **폐기**
   (`paper_html_build.el` 삭제). 검수 교훈: `raw[cite:]==0` 만 보면 안 됨(GPT가 잡음) → `csl-entry>0` + 문단 spot-check 필수.
   보너스: 원문의 깨진 `??` figure 참조를 `data-fignum` 으로 **"Figure N" 222개 복원**(HTML·PDF 공통).
...
5. [ ] (선택) truncated 섹션 figure 참조(남은 `??` 164) — 원문에 대상 없음. HTML CSL 스타일 커스터마이즈(현재 chicago-author-date).
