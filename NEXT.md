...
### 로드맵 (완료 정의)
1. [ ] **변환 로직 완성** — J-space 논문 1개를 org로 제대로 가져온다. pandoc(골격)+LLM(수식/구조/위젯 대체)+커스텀 조합. **재현가능하게 캡처**(도구·규칙·판단 전부).
2. [ ] **org → HTML 역변환 왕복 확인** — "논문 쓰기 포맷 = org" 성립 여부(ox-html/자체 템플릿).
3. [ ] **스킬화** — 위 로직/규칙을 `.claude/skills/`에 에이전트용 스킬로 추출.
4. [ ] **jacobian-lens 담당자 세팅** — GLG가 담당자 세우고, 담당자가 **스킬 보고 논문 가져와 repo에 담기.**
5. **완료 상태** = memex-kb가 **변환 로직 + 에이전트 스킬 + 실제 검수(QA)** 를 다 소유.
