...
- [ ] 책 config에 `paragraph_merge.enabled:true` 켜고 → `.merges.log` 통독 검수 → 틀린 seam은
      `overrides`로, glued 단어는 `literal`로 → 재빌드 `nix develop --command ./run.sh org2epub-build` → epubcheck 0/0/0.
- [ ] 검수에서 드러난 오탐 패턴을 seam 휴리스틱에 환류(특히 `이`-종결 모호건).
