...
### 다음 한 걸음 (의존 순서)
- [ ] **MVP: Threads 발행 수직슬라이스** — 가든 org → 요약형 포맷터 → Threads publish API. 이걸로 "포맷터/퍼블리셔" 인터페이스 확정.
- [ ] 가든 canonical org 입력 규약 정하기(autholog 태그, 원문 내장 위치, 가든 링크 자동 삽입·검증). **새 노트 금지·기존 노트 확장**(이슈 방침) → denotecli/semantic-memory 매칭으로 붙일 노트 찾기.
- [ ] 3종 포맷터 모듈화(원문형/전문형/요약형) + 매체별 글자수 테이블.
- [ ] Bluesky 퍼블리셔(API-clean 두번째).
- [ ] 네이버/티스토리 브라우저 자동화 PoC(`browser-tools` or 크롬확장).
- [ ] Facebook Page 퍼블리셔.
- [ ] 스킬 패키징 → 봇 주입. run.sh 명령 노출, AGENTS.md/README/BACKENDS 동기화.

> ⚠️ 검토 메모: 면들이 **API-clean vs 브라우저-자동화-강제** 두 종족으로 갈린다 — 클로드 브라우저 우회는 네이버/티스토리엔 선택이 아니라 유일경로. 아키텍처는 canonical-org → 포맷터 → 퍼블리셔 3겹, 퍼블리셔만 면별로 교체.
