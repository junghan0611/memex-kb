...
- `paper2org`: Anthropic Distill HTML → reproducible Org capture.
- `paper2org-html`: Org → Pandoc citeproc HTML with MathJax and rendered bibliography.
- `paper2org-pdf`: Org → acmart PDF via Emacs/TeX Live.
- Repo-local skill: `.claude/skills/anthropic-paper2org/SKILL.md`.

Open follow-ups:

1. [ ] `~/repos/gh/jacobian-lens/AGENTS.md` consumer handoff — keep conversion logic in memex-kb, write paper artifacts under `papers/anthropic/jspace/` in the consumer repo.
2. [ ] Optional DOCX export: evaluate Pandoc Org → DOCX or the existing Google Docs import path.
3. [ ] Optional TeX Live slimming: replace `scheme-full` with a curated acmart dependency set only if the dependency chase stays small.
4. [ ] Optional remaining `??` review: current unresolved refs point to figures absent from the captured source; leave as-is unless a concrete source target is found.
