...
2. [x] (2026-07-07) **web HTML 완성** — `paper2org-html` + `scripts/paper_html_build.el`(ox-html + oc-basic). web org 에
   `* References`/`#+print_bibliography:` 추가. 검증: **raw `[cite:` 0(인용 렌더)**, 참고문헌 목록 생성, MathJax 수식·이미지 정상.
   texlive 불필요(emacs 만). pandoc 왕복=증명, ox-html=프로덕션 = 둘 다 확보.
...
5. [ ] (선택) HTML CSL 서지(`#+cite_export: csl`+citeproc-el), 깨진 `??` 참조 평문화.
