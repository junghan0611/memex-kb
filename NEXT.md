...
- [ ] 위 노트로 `./run.sh syndicate` 묶음 만들어 8면 복붙본 점검 (네이버 서식·요약형 글자수 실측).

**링크 질문 해결(2026-06-15)**: 링크드인 `lnkd.in` 숏링크는 **문제없음** — 리다이렉트 래퍼라 목적지는 그대로 가든. canonical 수렴 유지. 단 (a) 링크드인은 본문 외부링크 글에 reach를 깎음 → 도달 원하면 link-in-comment, (b) 숏링크는 가든 도메인이 글자로 안 보임(사소). 둘 다 링크 무결성 문제 아님. cf. 블루스카이/트위터는 가든 URL **전체**가 글자수 차지(자동단축 없음/ t.co 23자).
