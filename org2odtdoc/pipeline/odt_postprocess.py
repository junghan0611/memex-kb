...
def strip_caption_numbers(content_xml_path):
    """캡션 자동 번호(sequence) 제거: '표 1: 제목' → '<표> 제목'.

    minirnd 양식은 캡션에 번호를 쓰지 않고 '<표>'/'<그림>' 라벨 + 제목만 둔다.
    ox-odt 가 생성한 '표 <text:sequence>N</text:sequence>: 제목' 패턴에서
    sequence 필드와 구분자(콜론)를 제거하고 라벨을 꺾쇠 형태로 바꾼다.
    """
    with open(content_xml_path, encoding="utf-8") as f:
        s = f.read()
    pat = re.compile(
        r"(표|그림)(?:&#160;|\s| )*"
        r"<text:sequence\b[^>]*>[^<]*</text:sequence>"
        r"(?:&#160;|\s| )*:?(?:&#160;|\s| )*"
    )
    s2, n = pat.subn(r"&lt;\1&gt; ", s)
    if n:
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        caps = strip_caption_numbers(content_path)
        print(f"캡션 번호 {caps}개 제거 (표/그림 → 라벨만)")
